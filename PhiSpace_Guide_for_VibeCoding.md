...
Use this guide as a skill-style operating reference when working on the PhiSpace package code, documentation, tests, or examples. The source of truth is the package under `pkg/`.

### 0.1 When to Use This Guide

Use this guide when the task involves:

- Adding, changing, or debugging exported PhiSpace functions.
- Updating roxygen documentation, man pages, `NAMESPACE`, `DESCRIPTION`, vignettes, or pkgdown-facing content.
- Changing how PhiSpace scores are computed, normalized, stored in `reducedDim()`, smoothed, visualized, clustered, or ranked.
- Debugging package install, test, document, or check failures.
- Integrating new utilities into `PhiSpace()` or `PhiSpaceR_1ref()`.

For one-off biological analyses that only call PhiSpace without editing the package, use the API sections below as reference but avoid package-source edits.

### 0.2 First-Pass Orientation

Before editing, inspect the files directly relevant to the task:

```bash
# Fast source lookup
rg -n "functionName|argumentName|reducedDim|metadata|@export" pkg/R pkg/tests pkg/man

# Current package exports and metadata
sed -n '1,220p' pkg/NAMESPACE
sed -n '1,220p' pkg/DESCRIPTION

# Worktree state
git status --short
```

Do not assume generated files are current. `pkg/man/*.Rd` and `pkg/NAMESPACE` are generated by roxygen2 when roxygen2 is available.

### 0.3 Task Decision Tree

| Task type | Primary files | Required follow-up |
|-----------|---------------|--------------------|
| New exported function | `pkg/R/<function>.R` | Add roxygen `@export`, tests, docs/NAMESPACE regeneration |
| Internal helper only | Existing or new `pkg/R/*.R` | Keep unexported unless there is a clear public API reason |
| Change existing API behavior | Existing `pkg/R/*.R` | Update roxygen, tests, examples, and caveats in this guide |
| Add `PhiSpace()` integration | `pkg/R/PhiSpaceR.R`, possibly `pkg/R/PhiSpaceR_1ref.R` | Preserve existing behavior unless the user explicitly asks for automatic dispatch |
| New dependency | `pkg/DESCRIPTION` | Use namespace-qualified calls or roxygen imports consistently |
| Documentation-only update | Roxygen comments, vignettes, guide | Regenerate docs if tooling is available |
| Test-only update | `pkg/tests/testthat/` | Keep fixtures small and deterministic |

### 0.4 Package Invariants

- `PhiSpace()` is the high-level wrapper and stores normalized scores in `reducedDim(query, reducedDimName)`, default `"PhiSpace"`.
- `PhiSpaceR_1ref()` is the single-reference engine and returns both raw and normalized query/reference scores: `PhiSpaceScore`, `PhiSpaceNorm`, `YrefHat`, `YrefHatNorm`.
- `PhiSpace()` defaults to `refAssay = "rank"`; `PhiSpaceR_1ref()` defaults to `refAssay = "log1p"`. Set assays explicitly in examples and tests.
- Query score matrices are cells × phenotypes/classes. Preserve row names from query cells and informative column names from phenotype classes.
- New score-like outputs should use `reducedDim()` for cell × score matrices and `S4Vectors::metadata()` for auxiliary objects such as parameters, signatures, centroids, and diagnostics.
- Many functions accept SCE/SPE objects. Validate assays, `colData()` columns, `reducedDim()` names, dimensions, and missing values early.
- Use `stop()` for invalid inputs, `warning()` for recoverable risks, and `message()` for optional progress.
- Keep generated `pkg/man/*.Rd` and `pkg/NAMESPACE` synchronized with roxygen comments. Edit generated files manually only when roxygen2 is unavailable and note that they should be regenerated later.

### 0.5 Verification Commands

Run the strongest available verification path for the environment:

```bash
# Preferred full workflow, if devtools is installed
Rscript -e 'devtools::document("pkg")'
Rscript -e 'devtools::test("pkg")'
Rscript -e 'devtools::check("pkg")'

# Fallback if devtools is unavailable but roxygen2 is installed
Rscript -e 'roxygen2::roxygenise("pkg")'
R CMD INSTALL pkg
R CMD check --no-manual pkg

# Minimal fallback for constrained local libraries
R CMD INSTALL -l /private/tmp/phispacelib pkg
Rscript -e '.libPaths(c("/private/tmp/phispacelib", .libPaths())); library(PhiSpace); packageVersion("PhiSpace")'
Rscript -e 'tools::checkRd("pkg/man/<function>.Rd")'
```

If `testthat`, `devtools`, or `roxygen2` is missing, do not silently skip verification. Run direct `Rscript` smoke checks against a temporary install and report the missing packages.

### 0.6 Testing Expectations

- Narrow utility functions such as `piScore()` need small table-driven tests for defaults, explicit arguments, auto-detection, `NA`, and edge cases.
- SCE/SPE functions such as `scoreCells()` need small synthetic fixtures that verify dimensions, row/column names, output slots, and metadata.
- Wrapper changes to `PhiSpace()` need regression tests showing existing behavior remains unchanged and new behavior only triggers under the intended arguments.
- Spatial functions should verify both `SpatialExperiment::spatialCoords()` and SCE `colData()` coordinate paths when coordinates are involved.
- Prefer deterministic fixtures over large external data. Manual integration tests under `Test/` may require ignored local datasets and should not be the only verification.
...
- Do not edit `docs/` pkgdown output directly unless the task is explicitly about generated site files.
- Do not install or restore dependencies into the project without checking the active library path and asking for approval when network access or external writes are required.
...
- `reducedDim(query, "scoreCells_correlation")`: query cell/bin correlation with each class centroid
- `reducedDim(query, "scoreCells_signature")`: class-signature activity scores
- `S4Vectors::metadata(query)$scoreCells`: signatures, marker statistics, centroids, classes, and run parameters

```r
# Multi-class reference: one score column per class in colData(reference)$cell_type
query <- scoreCells(
  reference   = reference_sce,
  query       = query_sce,
  class_col   = "cell_type",
  ...
  scoring     = "both",          # "both", "correlation", or "signature"
  cor_method  = "spearman",      # or "pearson"
  n_top_genes = 300,
  n_signature_genes = 100
)

cor_scores <- reducedDim(query, "scoreCells_correlation")
sig_scores <- reducedDim(query, "scoreCells_signature")
```

**Single-class reference mode**:

```r
# All reference cells are treated as one class called "reference"
query <- scoreCells(
...
score <- reducedDim(query, "scoreCells_correlation")[, "reference"]
```
...
VizSpatial(query, reducedDim = "reference",
           reducedDim2use = "scoreCells_signature_smoothed")
```

**Marker metadata and signatures**:

```r
signatures  <- S4Vectors::metadata(query)$scoreCells$signatures
marker_stat <- S4Vectors::metadata(query)$scoreCells$marker_stats
centroids   <- S4Vectors::metadata(query)$scoreCells$centroids
```
...
### 3.6 Filtering Rare Cell Types: `cellTypeThreshold`
...
### 3.7 Parameter Tuning: `tunePhiSpace()`
...
### 6.2 Differential Expression Ranking: `piScore()`

`piScore()` combines log fold change and p-value into a single gene-ranking statistic. The default is the signed PhiSpace convention:

```
pi_score = log2FC * -log10(p)
```

Positive values rank up-regulated genes; negative values rank down-regulated genes. Set `signed = FALSE` for the unsigned Xiao et al. (2014) definition.
...
The function auto-detects common column conventions:

| Tool/output | logFC column | p-value column |
|-------------|--------------|----------------|
| Seurat `FindMarkers` | `avg_log2FC` | `p_val_adj` or `p_val` |
| `scran::findMarkers` | `summary.logFC` | `FDR` |
| limma / edgeR | `logFC` | `adj.P.Val` or `FDR` |
| DESeq2 | `log2FoldChange` | `padj` or `pvalue` |

```r
# Explicit columns always override detection
markers <- piScore(
  markers,
...
# Return just a vector instead of appending columns
scores <- piScore(markers, add_column = FALSE)
```

`p = 0` values are clamped to `pval_floor` before `-log10(p)` to avoid infinite scores. `NA` values in logFC or p-values remain `NA`.

### 6.3 Cell Type Correlation: `cellTypeCorMat()`
...
| Parameter | Default | Notes |
|-----------|---------|-------|
| `reference` | required | SCE/SPE reference object |
| `query` | required | SCE/SPE query object |
| `class_col` | `NULL` | Reference class column; `NULL` treats all reference cells as one class |
| `assay_ref` | `"logcounts"` | Reference assay |
| `assay_query` | `"logcounts"` | Query assay |
| `signature_method` | `"findMarkers"` | Uses `scran::findMarkers()` for multi-class; automatically falls back to mean ranking for one class |
| `marker_test` | `"t"` | `"t"`, `"wilcox"`, or `"binom"` |
| `n_top_genes` | `300` | Candidate genes before housekeeping removal |
| `n_signature_genes` | `100` | Final signature genes per class |
| `remove_housekeeping` | `TRUE` | Removes ribosomal, mitochondrial, and built-in housekeeping genes |
| `scoring` | `"both"` | `"both"`, `"correlation"`, or `"signature"` |
| `cor_method` | `"spearman"` | `"spearman"` or `"pearson"` |
| `zscore_signature` | `TRUE` | Across classes per cell; across cells for one-class references |
| `score_prefix` | `"scoreCells"` | Prefix for output reducedDim names |

### `piScore()`

| Parameter | Default | Notes |
|-----------|---------|-------|
| `de_results` | required | data.frame-like DE table |
| `logfc_col` | `"avg_log2FC"` | Auto-detects common alternatives if absent |
| `pval_col` | `"p_val_adj"` | Auto-detects common alternatives if absent |
| `signed` | `TRUE` | Directional PhiSpace convention; `FALSE` uses unsigned Xiao et al. definition |
| `pval_floor` | `.Machine$double.xmin` | Clamp p-values below this floor |
| `add_column` | `TRUE` | Append `pi_score`; otherwise return numeric vector |
| `rank` | `FALSE` | Append `pi_rank`, descending by `pi_score` |
...
```
reducedDim(query, "scoreCells_correlation") # correlation scores: cells × reference classes
reducedDim(query, "scoreCells_signature")   # signature scores: cells × reference classes
S4Vectors::metadata(query)$scoreCells       # signatures, marker_stats, centroids, params
```
...
## 12. Exported Functions Reference (40 exports)
...
| `scoreCells()` | Core | Correlation/signature fallback for few-class or single-class references |
...
| `piScore()` | Analysis | Signed or unsigned pi-score for DE gene ranking |
...
```
pkg/
├── R/           # 43 R source files (edit these)

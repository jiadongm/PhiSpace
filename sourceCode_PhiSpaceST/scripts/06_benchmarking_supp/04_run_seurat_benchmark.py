#!/usr/bin/env python3
"""
Orchestration script for Seurat benchmarking.
Runs Seurat on all 32 simulated datasets and computes metrics.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run full Seurat benchmarking pipeline')
    parser.add_argument('--output_dir', type=str,
                        default='results_seurat',
                        help='Output directory for results (relative to script dir)')
    parser.add_argument('--datasets', type=str, default='all',
                        help='Comma-separated dataset numbers or "all" (1-32)')
    parser.add_argument('--skip_seurat', action='store_true',
                        help='Skip Seurat execution step (only compute metrics)')
    ...
    print("="*60)
    print("Seurat Full Benchmarking Pipeline")
    print("="*60)
    ...
    # Step 1: Run Seurat on all datasets
    if not args.skip_seurat:
        print("\n" + "#"*60)
        print("# STEP 1: Running Seurat")
        print("#"*60)
        ...
        for ds_num in datasets:
            ...
            cmd = f"Rscript {script_dir / '02_run_seurat.R'} " \
                  f"{ds_prepared_dir} {ds_output_dir}"

            success, elapsed = run_command(cmd, f"Running Seurat on dataset {ds_num}")
    ...
    for ds_num in datasets:
        ...
        seurat_result = ds_output_dir / "Seurat_result.txt"

        if not gt_path.exists():
            print(f"WARNING: Ground truth for dataset {ds_num} not found")
            continue

        if not seurat_result.exists():
            print(f"WARNING: Seurat result for dataset {ds_num} not found")
            continue
    ...
    for metric, dfs in all_metrics.items():
        if dfs:
            ...
            output_file = output_dir / f"Seurat_{metric.upper()}.csv"
            ...
            for df in dfs:
                ...
                if 'Seurat' in mean_val.index:
                    summary_rows.append({
                        'dataset': ds_num,
                        'metric': metric.upper(),
                        'mean': mean_val['Seurat']
                    })
    ...
    if summary_rows:
        ...
        summary_wide.to_csv(output_dir / 'Seurat_all_metrics.csv')
        print(f"\nAll metrics summary saved to {output_dir / 'Seurat_all_metrics.csv'}")
    ...
    if timing_results:
        ...
        timing_file = output_dir / "seurat_timing.csv"
    ...
    print("\n" + "="*60)
    print("Seurat Benchmarking Pipeline Completed!")
    print("="*60)

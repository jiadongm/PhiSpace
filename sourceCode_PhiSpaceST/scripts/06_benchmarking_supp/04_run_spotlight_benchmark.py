#!/usr/bin/env python3
"""
Orchestration script for SPOTlight benchmarking.
Runs SPOTlight on all 32 simulated datasets and computes metrics.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run full SPOTlight benchmarking pipeline')
    parser.add_argument('--output_dir', type=str,
                        default='results_spotlight',
                        help='Output directory for results (relative to script dir)')
    parser.add_argument('--datasets', type=str, default='all',
                        help='Comma-separated dataset numbers or "all" (1-32)')
    parser.add_argument('--skip_spotlight', action='store_true',
                        help='Skip SPOTlight execution step (only compute metrics)')
    ...
    print("="*60)
    print("SPOTlight Full Benchmarking Pipeline")
    print("="*60)
    ...
    # Step 1: Run SPOTlight on all datasets
    if not args.skip_spotlight:
        print("\n" + "#"*60)
        print("# STEP 1: Running SPOTlight")
        print("#"*60)
        ...
        for ds_num in datasets:
            ...
            cmd = f"Rscript {script_dir / '02_run_spotlight.R'} " \
                  f"{ds_prepared_dir} {ds_output_dir}"

            success, elapsed = run_command(cmd, f"Running SPOTlight on dataset {ds_num}",
                                           timeout=7200)
    ...
    for ds_num in datasets:
        ...
        spotlight_result = ds_output_dir / "SPOTlight_result.txt"

        if not gt_path.exists():
            print(f"WARNING: Ground truth for dataset {ds_num} not found")
            continue

        if not spotlight_result.exists():
            print(f"WARNING: SPOTlight result for dataset {ds_num} not found")
            continue
    ...
    for metric, dfs in all_metrics.items():
        if dfs:
            ...
            output_file = output_dir / f"SPOTlight_{metric.upper()}.csv"
            ...
            for df in dfs:
                ...
                if 'SPOTlight' in mean_val.index:
                    summary_rows.append({
                        'dataset': ds_num,
                        'metric': metric.upper(),
                        'mean': mean_val['SPOTlight']
                    })
    ...
    if summary_rows:
        ...
        summary_wide.to_csv(output_dir / 'SPOTlight_all_metrics.csv')
        print(f"\nAll metrics summary saved to {output_dir / 'SPOTlight_all_metrics.csv'}")
    ...
    if timing_results:
        ...
        timing_file = output_dir / "spotlight_timing.csv"
    ...
    print("\n" + "="*60)
    print("SPOTlight Benchmarking Pipeline Completed!")
    print("="*60)

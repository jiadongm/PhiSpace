#!/usr/bin/env python3
"""
Orchestration script for TACCO benchmarking.
Runs TACCO on all 32 simulated datasets and computes metrics.
"""
...
TACCO_PYTHON = paths["tacco_python"]
...
def main():
    parser = argparse.ArgumentParser(description='Run full TACCO benchmarking pipeline')
    parser.add_argument('--output_dir', type=str,
                        default='PhiSpace_Benchmarking/results_tacco',
    ...
    print("="*60)
    print("TACCO Full Benchmarking Pipeline")
    print("="*60)
    ...
    print(f"TACCO Python: {TACCO_PYTHON}")

    # Track timing and results
    timing_results = []

    # Step 1: Run TACCO on all datasets
    if not args.skip_deconv:
        print("\n" + "#"*60)
        print("# STEP 1: Running TACCO")
        print("#"*60)
        ...
        for ds_num in datasets:
            ...
            cmd = f"{TACCO_PYTHON} {script_dir / '02_run_tacco.py'} " \
                  f"{ds_prepared_dir} {ds_output_dir}"

            success, elapsed = run_command(cmd, f"Running TACCO on dataset {ds_num}")
    ...
    for ds_num in datasets:
        ...
        result_file = ds_output_dir / "TACCO_result.txt"
        ...
        if not result_file.exists():
            print(f"WARNING: TACCO result for dataset {ds_num} not found")
            continue
    ...
    for metric, dfs in all_metrics.items():
        if dfs:
            ...
            output_file = output_dir / f"TACCO_{metric.upper()}.csv"
    ...
    if summary_rows:
        ...
        summary_wide.to_csv(output_dir / 'TACCO_all_metrics.csv')
        print(f"\nAll metrics summary saved to {output_dir / 'TACCO_all_metrics.csv'}")
    ...
    if timing_results:
        ...
        timing_file = output_dir / "tacco_timing.csv"
    ...
    print("\n" + "="*60)
    print("TACCO Benchmarking Pipeline Completed!")
    print("="*60)

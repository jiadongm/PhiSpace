...
matrixPlot <- function(
  ...
  # Create a mapping between original and temporary names
  ...
  if(!all(requested_cols %in% colnames(scores))){
    missing_idx <- comp_idx[!(requested_cols %in% colnames(scores))]
    stop(paste0("These component indices are out of range: ", paste(missing_idx, collapse = ", "),
                ". Matrix only has ", ncol(scores), " columns."))
  }

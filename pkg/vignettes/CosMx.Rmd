...
if(!file.exists(PhiResPath)){
  ...
  for(ii in 1:length(ref_list)){
    ...
    PhiRes <- PhiSpaceR_1ref(
      reference, query, 
      phenotypes = YtrainName, refAssay = "log1p",
...
```{r fig.asp=1}
VizSpatial(query, "sdimx", "sdimy", ptSize = 1, colBy = "niche", legend.position = "top", legend.symb.size = 2) + 
...
VizSpatial(
  query, x_coord = "sdimx", y_coord = "sdimy", colBy = "PhiClust", ptSize = 0.5
...
VizSpatial(
  query, "sdimx", "sdimy", ptSize = 0.5, colBy = "divergence", reOrder = T, fsize = 12
) +
...
dwd_res$scores %>%
  ...
  xlab("DWD score")
plotLoadings(dwd_res$loadings, "comp1", xlab = "DWD loading", nfeat = 10) +
  theme(

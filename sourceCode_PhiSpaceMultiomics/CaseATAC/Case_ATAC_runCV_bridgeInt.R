...
# Normalise (obj.multi.batches have already been normalised)

multiPath <- paste0(dat_dir, "data/NeurIPS2021/obj.multi.batches_forPhiSpace_cor.rds")

if(!file.exists(multiPath)){
  if(F){
    obj.rna <- SCTransform(obj.rna)
    DefaultAssay(obj.rna) <- "SCT"
    saveRDS(obj.rna, paste0(dat_dir, "data/NeurIPS2021/obj.rna_for_refMap.rds"))
  }
  DefaultAssay(obj.rna) <- "RNA"
  obj.rna[["SCT"]] <- NULL
  for(i in 1:length(obj.multi.batches)){
    DefaultAssay(obj.multi.batches[[i]]) <- "RNA"
    obj.multi.batches[[i]][["SCT"]] <- NULL
  }
  ...
  features <- Reduce(base::intersect, features)
  features <-
    intersect(
      features,
      rownames(obj.rna)
  ...
  obj.rna <- SCTransform(obj.rna[features,])
  DefaultAssay(obj.rna) <- "SCT"
  ...
  for(ii in 1:length(obj.multi.batches)){
    ...
    obj.multi <- obj.multi[features, ]
    DefaultAssay(obj.multi) <- "RNA"
    obj.multi@meta.data['log_umi'] <- log10(obj.multi$nCount_RNA)
    
    obj.multi <- SCTransform(
      obj.multi, 
      reference.SCT.model = slot(
        object = obj.rna[["SCT"]], 
        name = "SCTModel.list"
      )[[1]]
    )
...
saveRDS(bridgeIntAnn_cv, paste0(dat_dir, "output/Case2/bridgeIntAnn_cv_cor.rds"))